...
static void benchmark_schnorr_init(void* arg) {
    ...
    for (k = 0; k < data->numsigs; k++) {
        ...
        CHECK(secp256k1_ec_pubkey_createOP(data->ctx, &pubkey, data->sigs[k].key));
        CHECK(secp256k1_ec_pubkey_serializeOP(data->ctx, data->sigs[k].pubkey, &data->sigs[k].pubkeylen, &pubkey, SECP256K1_EC_COMPRESSED));
    }
}
...
static void benchmark_schnorr_verify(void* arg) {
    ...
    for (i = 0; i < 20000 / data->numsigs; i++) {
        ...
        CHECK(secp256k1_ec_pubkey_parseOP(data->ctx, &pubkey, data->sigs[0].pubkey, data->sigs[0].pubkeylen));
