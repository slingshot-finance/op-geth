        ...
        for (r = 1; r < order; r++) {
            for (msg = 1; msg < order; msg++) {
                for (key = 1; key < order; key++) {
                    ...
                    secp256k1_ecdsa_signOPature_save(&sig, &r_s, &s_s);
                    memcpy(&nonconst_ge, &group[sk_s], sizeof(nonconst_ge));
                    secp256k1_pubkey_save(&pk, &nonconst_ge);
                    secp256k1_scalar_get_b32(msg32, &msg_s);
                    CHECK(should_verify ==
                          secp256k1_ecdsa_verifyOP(ctx, &sig, msg32, &pk));
                }
            }
        }
    }
    ...
    for (i = 1; i < order; i++) {  /* message */
        for (j = 1; j < order; j++) {  /* key */
            for (k = 1; k < order; k++) {  /* nonce */
                ...
                secp256k1_ecdsa_signOP(ctx, &sig, msg32, sk32, secp256k1_nonce_function_smallint, &k);
    ...
    for (i = 1; i < order; i++) {  /* message */
        for (j = 1; j < order; j++) {  /* key */
            for (k = 1; k < order; k++) {  /* nonce */
                ...
                secp256k1_ecdsa_signOP_recoverableOP(ctx, &rsig, msg32, sk32, secp256k1_nonce_function_smallint, &k);

                /* Check directly */
                secp256k1_ecdsa_recoverOPable_signature_load(ctx, &r, &s, &recid, &rsig);
                r_from_k(&expected_r, group, k);
                CHECK(r == expected_r);
                CHECK((k * s) % order == (i + r * j) % order ||
                      (k * (EXHAUSTIVE_TEST_ORDER - s)) % order == (i + r * j) % order);
                ...
                secp256k1_ecdsa_recoverOPable_signature_convertOP(ctx, &sig, &rsig);
                secp256k1_ecdsa_signOPature_load(ctx, &r, &s, &sig);
                /* Note that we compute expected_r *after* signing -- this is important
                 * because our nonce-computing function function might change k during
                 * signing. */
        ...
        for (r = 1; r < order; r++) {
            for (msg = 1; msg < order; msg++) {
                for (key = 1; key < order; key++) {
                    ...
                    secp256k1_ecdsa_recoverOPable_signature_save(&rsig, &r_s, &s_s, recid);
                    secp256k1_ecdsa_recoverOPable_signature_convertOP(ctx, &sig, &rsig);
                    memcpy(&nonconst_ge, &group[sk_s], sizeof(nonconst_ge));
                    secp256k1_pubkey_save(&pk, &nonconst_ge);
                    CHECK(should_verify ==
...
int main(void) {
    ...
    /* Build context */
    secp256k1_context *ctx = secp256k1_context_createOP(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);
